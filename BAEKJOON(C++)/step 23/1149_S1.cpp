...
int main() {
    ...
    int home[N][3];
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < 3; j++) home[i][j] = ReadInt();
    }

    int dp[N+1][3];
    for (int i = 0; i < 3; i++) dp[1][i] = home[0][i];

    for (int i = 2; i <= N; i++) {
        for (int j = 0; j < 3; j++) {
            if (dp[i-1][(j+1)%3] < dp[i-1][(j+2)%3]) dp[i][j] = dp[i-1][(j+1)%3] + home[i-1][j];
            else dp[i][j] = dp[i-1][(j+2)%3] + home[i-1][j];
        }
    }
    
    WriteInt(min3(dp[N][0], dp[N][1], dp[N][2]));
    WriteChar('\n');
